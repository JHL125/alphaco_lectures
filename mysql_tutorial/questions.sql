...
select * from tb_point;
select * from tb_point
where POINT between 10000 and 50000 and REG_DTTM like '2018%';
SELECT *
FROM   TB_POINT
...
select CUSTOMER_CD as 고객코드
    ...
    ,TOTAL_POINT as 누적포인트
from tb_customer 
where MW_FLG = 'M' and BIRTH_DAY in (5,6,7);
...
select SALES_DT as 판매일수, PRODUCT_NM as 상품명, sum(SALES_COUNT) as 총판매수
from tb_sales
group by SALES_DT, PRODUCT_NM
having SALES_DT in('20190802', '20190803')
order by 1, 2;
